...
from app.schemas.auth import (
    ...
    GoogleLoginRequest, FacebookLoginRequest, LinkedInLoginRequest
)
...
async def login_json(
    ...
    request: Request,
    db: AsyncSession = Depends(get_db_with_retry)  # Use retry for critical auth
):
    ...
    if login_data.recaptcha_token:
        ...
        recaptcha_result = await verify_recaptcha(login_data.recaptcha_token, client_ip)
        
        if not recaptcha_result.get("success"):
            logger.warning(f"reCAPTCHA verification failed for {login_data.email}: {recaptcha_result.get('error')}")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="reCAPTCHA verification failed. Please try again.",
            )
        
        logger.info(f"reCAPTCHA verified successfully for login attempt: {login_data.email}")
    else:
        # If reCAPTCHA is configured, require it
...
async def signup(
    ...
    request: Request,
    db: AsyncSession = Depends(get_db_with_retry)  # Use retry for critical registration
):
    """Register new user account and send verification email - data stored temporarily until verification"""

    # Verify reCAPTCHA token if provided
    if hasattr(user_data, 'recaptcha_token') and user_data.recaptcha_token:
        client_ip = request.client.host if request.client else None
        recaptcha_result = await verify_recaptcha(user_data.recaptcha_token, client_ip)
        
        if not recaptcha_result.get("success"):
            logger.warning(f"reCAPTCHA verification failed for signup: {user_data.email}: {recaptcha_result.get('error')}")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="reCAPTCHA verification failed. Please try again.",
            )
        
        logger.info(f"reCAPTCHA verified successfully for signup: {user_data.email}")
    else:
        # If reCAPTCHA is configured, require it
        if hasattr(settings, 'RECAPTCHA_SECRET_KEY') and settings.RECAPTCHA_SECRET_KEY:
            raise HTTPException(
    ...
    # Handle email verification - allow signup without email service until configured
    if not email_sent:
        if not settings.POSTMARK_SERVER_TOKEN:
            # Email service not configured - allow signup and log OTP for manual verification
            logger.warning(f"EMAIL SERVICE DISABLED - OTP for {temp_user.email}: {otp}")
            logger.info(f"Manual verification available - OTP: {otp}")
            logger.info(f"To verify manually, use OTP: {otp} for email: {temp_user.email}")
        else:
    ...
    try:
        ...
        if user:
            ...
            user = User(
                ...
                full_name=name,
                google_id=google_id,
                avatar_url=None,  # Use default profile pic
                is_email_verified=email_verified,  # Google emails are verified
                is_active=True,
...
async def facebook_login(
    ...
    """Login/signup using Facebook token"""
    
    try:
        # Verify Facebook token
        facebook_url = f"https://graph.facebook.com/me?access_token={request.access_token}&fields=id,name,email,picture"
        response = http_requests.get(facebook_url)
        ...
        result = await db.execute(
            select(User).where(
                or_(User.email == email, User.facebook_id == facebook_id)
            )
        )
        user = result.scalar_one_or_none()
        ...
        if user:
            ...
            user = User(
                ...
                avatar_url=None,  # Use default profile pic
                is_email_verified=True,  # Facebook emails are considered verified
                is_active=True,
                hashed_password=get_password_hash(secrets.token_urlsafe(32))  # Random password for OAuth users
            )
            db.add(user)
...
async def linkedin_login(
    ...
    """Login/signup using LinkedIn authorization code"""
    ...
    try:
        ...
        if token_response.status_code != 200:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Invalid LinkedIn authorization code"
            )
        ...
        profile_url = "https://api.linkedin.com/v2/people/~:(id,firstName,lastName,emailAddress)"
        profile_response = http_requests.get(profile_url, headers=headers)
        ...
        if not email:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Email permission required for LinkedIn login"
            )
        
        # Check if user exists
        result = await db.execute(
            select(User).where(
                or_(User.email == email, User.linkedin_id == linkedin_id)
            )
        )
        user = result.scalar_one_or_none()
        ...
        if user:
            ...
            user = User(
                ...
                avatar_url=None,  # Use default profile pic
                is_email_verified=True,  # LinkedIn emails are considered verified
                is_active=True,
                hashed_password=get_password_hash(secrets.token_urlsafe(32))  # Random password for OAuth users
            )
            db.add(user)
