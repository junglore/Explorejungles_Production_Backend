...
def upgrade() -> None:
    ...
    transaction_type_enum = postgresql.ENUM('POINTS_EARNED', 'CREDITS_EARNED', 'CREDITS_SPENT', 'POINTS_PENALTY', 'CREDITS_PENALTY', 'ADMIN_ADJUSTMENT', name='transactiontypeenum', create_type=False)
    transaction_type_enum.create(conn, checkfirst=True)
    
    currency_type_enum = postgresql.ENUM('POINTS', 'CREDITS', name='currencytypeenum', create_type=False)
    currency_type_enum.create(conn, checkfirst=True)
    
    activity_type_enum = postgresql.ENUM('QUIZ_COMPLETION', 'MYTHS_FACTS_GAME', 'DAILY_LOGIN', 'STREAK_BONUS', 'ACHIEVEMENT_UNLOCK', 'ADMIN_GRANT', 'PURCHASE', 'REFUND', name='activitytypeenum', create_type=False)
    activity_type_enum.create(conn, checkfirst=True)
    
    reward_activity_type_enum = postgresql.ENUM('QUIZ_COMPLETION', 'MYTHS_FACTS_GAME', 'DAILY_LOGIN', 'STREAK_BONUS', 'ACHIEVEMENT_UNLOCK', name='rewardactivitytypeenum', create_type=False)
    reward_activity_type_enum.create(conn, checkfirst=True)
    
    reward_tier_enum = postgresql.ENUM('BRONZE', 'SILVER', 'GOLD', 'PLATINUM', name='rewardtierenum', create_type=False)
    reward_tier_enum.create(conn, checkfirst=True)
    
    achievement_type_enum = postgresql.ENUM('QUIZ_MASTER', 'MYTH_BUSTER', 'SPEED_DEMON', 'PERFECT_SCORE', 'DAILY_WARRIOR', 'WEEK_STREAK', 'MONTH_STREAK', 'QUIZ_CHAMPION', name='achievementtypeenum', create_type=False)
    achievement_type_enum.create(conn, checkfirst=True)
    
    leaderboard_type_enum = postgresql.ENUM('GLOBAL_POINTS', 'GLOBAL_QUIZ', 'GLOBAL_MYTHS', 'WEEKLY_POINTS', 'MONTHLY_POINTS', 'CATEGORY_SPECIFIC', name='leaderboardtypeenum', create_type=False)
    leaderboard_type_enum.create(conn, checkfirst=True)
    
    anti_gaming_activity_enum = postgresql.ENUM('QUIZ_COMPLETION', 'MYTHS_FACTS_GAME', name='antigamingactivityenum', create_type=False)
    anti_gaming_activity_enum.create(conn, checkfirst=True)
    
    quiz_reward_tier_enum = postgresql.ENUM('BRONZE', 'SILVER', 'GOLD', 'PLATINUM', name='quizrewardtierenum', create_type=False)
    quiz_reward_tier_enum.create(conn, checkfirst=True)
